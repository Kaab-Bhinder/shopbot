...
const Banner = () => {
  ...
  const handleImageError = (imagePath: string) => {
    console.error(`Failed to load banner image: ${imagePath}`);
  ...
  return (
    <section className="relative w-full h-screen">
          ...
          >
            {!imageError[slide.image] ? (
              <div className="w-full h-full relative">
                <Image
                  src={slide.image}
                  alt={`${slide.title} - Fashion banner showing ${slide.subtitle}`}
                  fill
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="bg-gray-200 bg-opacity-60 w-1/2 py-8 px-6 text-center">
                <h1 className="text-5xl font-bold mb-4 text-gray-900 drop-shadow-lg">
                  {slide.title}
                </h1>
                <p className="text-2xl font-medium text-gray-800 drop-shadow-lg">
                  {slide.subtitle}
                </p>
              </div>
            </div>
      ...
      <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 flex space-x-3">
        {bannerData.map((_, index) => (
          <button
            ...
            className={`w-3 h-3 rounded-full transition-all duration-300 ${index === currentSlide ? 'bg-white scale-125' : 'bg-white opacity-50 hover:opacity-75'}`}
          />
        ))}
      </div>
      ...
      {/* Navigation Arrows */}
      <button
        onClick={() => goToSlide((currentSlide - 1 + bannerData.length) % bannerData.length)}
      ...
      <button
        onClick={() => goToSlide((currentSlide + 1) % bannerData.length)}
      ...
      <div className="absolute bottom-8 right-8">
        <button className="bg-green-500 hover:bg-green-600 text-white rounded-full p-4 shadow-lg transition-colors">
